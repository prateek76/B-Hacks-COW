    	...
    	if($method === 'getstudentApps') {
			if(/*!student*/false) {//testing
				...
				try {
		        	...
		        	$stmt = $connection->prepare('SELECT * FROM applications WHERE rollNumber = :roll AND mobile = :mobile ORDER BY status ASC');
		    		$stmt->execute(['roll' => $_SESSION['enroll'], 'mobile' => $_SESSION['HOSTEL']]);
