<?php 
    ...
    if(!isset($_POST['submit'])){
        ...
        try {
        	...
        	$stmt = $connection->prepare('SELECT * FROM users WHERE Email = :email AND Enroll = :enroll AND HOSTEL = :HOSTEL AND Password = :password');
    		$stmt->execute(['email' => $email, 'password' => $pass,'enroll' => $enroll,'HOSTEL' => $HOSTEL]);
