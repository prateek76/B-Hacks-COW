    ...
    if(!empty($_POST['submit'])) {
      ...
      //  echo '<script language="javascript"> alert("all fields required") </script>'; 
       // header("Refresh: 1; url=../student_dashboard.php");
    	...
    	$natureOfLeave = htmlentities($_POST['IFSC_CODE']);
    	$purpose = htmlentities($_POST['BANK_NAME']);
    	$classScheduledOnLeave = htmlentities($_POST['BANK_HOLDER_NAME']);
    	$address = 	htmlentities($_POST['BANK_BRANCH_NAME']);
    	//$mobile = htmlentities($_POST['HOSTEL']);
    	//$email = htmlentities($_POST['email']);
    	$uploadedImageName =   "";

    	# backend check for empty fields
    	if(empty(trim($studentName)) || empty(trim($branch)) || empty(trim($semester)) || empty(trim($startDate)) || empty(trim($endDate)) || empty(trim($natureOfLeave)) || empty(trim($purpose)) || empty(trim($classScheduledOnLeave)) || empty(trim($address)) ) {
