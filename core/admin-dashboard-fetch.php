    	...
    	if($method === 'getApps') {
			if(/*!admin*/false) {//testing
				...
				try {
		        	...
		        	$stmt = $connection->prepare('SELECT * FROM applications WHERE mobile = :branch ORDER BY status ASC');
				...
				if(/*!admin*/false) {//testing
					...
					try {
				        	$connection->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				        	$stmt = $connection->prepare('SELECT * FROM applications WHERE mobile = :branch AND status = :status');
